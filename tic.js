...
const rows = (board) => {
   ...
   let line = "";
   for (i=0; i<=6; i=i+3){
   if(board.slice(i,i+3).join("") ==="XXX"){  
     winner = "X"
     line = `line${i}`
    } 
   if(board.slice(i,i+3).join("")  ==="OOO"){  
     winner = "O"
     line= `line${i}`
    }
   }
  ...
  const diagonals = (board) => {
    let line = ""
    let winner = "" 
    const diagLeft = board.reduce((a,b,index)=>
     index===4||index===8?a+b:a,board[0])
    const diagRight = board.reduce((a,b,index)=>
     index===4||index===6? a+=b:a,board[2])
    if(diagLeft === "XXX"){ line = "diagonal1"; winner = "X"}
    if(diagRight === "XXX"){ line = "diagonal2"; winner = "X"}
    if(diagLeft === "OOO"){ line = "diagonal1";  winner = "O"} 
    if(diagRight ==="OOO"){ line = "diagonal2"; winner = "O"}
    return {winner, line}
  }
  const columns = (board) => { 
  let winner = "";
  let line = "";
    for(i=0;i<=2;i++){
     ...
     if(column === "XXX"){ line = `column${i}`; winner = "X"}
     if(column === "OOO"){ line = `column${i}`; winner = "O"}
    }
  return {winner, line}
  }
  const turn = (board) => {
...
return {turn, freeMoves, over, result, drawLine}})()
